...
let run _quiet cfg filenames output =
  let ppf, finally = match output with
    ...
    | Some filename ->
        ...
        ppf, finally in
  Fun.protect ~finally @@ fun () ->
  let fn (filename, name) = match name with
    | None ->
...
let non_existing_filename filename =
  ...
  then error_msgf "%s already exists" filename
  else Ok ()
...
let safe_filename_as_name filename =
  ...
  let fn1 = function
    ...
    | '0' .. '9'
    | '\'' -> true
    | chr -> fn0 chr || to_underscore chr in
  if String.length filename > 0
  && fn0 filename.[0]
  && String.for_all fn1 filename
  then Ok () else error_msgf "%s is not a safe filename" filename
...
let safe_name name =
  ...
  && fn0 name.[0]
  && String.for_all fn1 name
  then Ok () else error_msgf "%s is not a safe name" name
...
let pp_of_arg ppf = function
  | (filename, None) ->
      if no_colon filename
      then Fmt.string ppf filename
      else Fmt.pf ppf "-:%s" filename
  | (filename, Some name) ->
      Fmt.pf ppf "%s:%s" name filename

let setup_filenames filenames =
  let fn = function
    | (_, Some name) -> name
    | (filename, None) -> filename_to_name filename in
  let names = List.map fn filenames in
  let rec has_duplicate = function
    | [] -> false
    | x :: r -> List.mem x r || has_duplicate r in
  if has_duplicate names
  then error_msgf "Found some duplications on names"
  else if List.is_empty names
  then error_msgf "No file specified to crunch"
  else Ok filenames

let filenames =
  let doc = "The file to $(i,crunch) into the OCaml output file. The user can \
             specify the OCaml name to obtain the contents of the file using \
             the separator $(i,:) (with the name on the left and the file on \
             the right). If the filename contains the character $(i,:) and the \
             user would like to let $(tname) infer the OCaml name, the user \
             can specify $(i,-:filename)." in
  let open Arg in
  value
...
let with_comments =
  let doc = "Print a human-readable view of each line of the contents as a comment." in
  let open Arg in
...
let kind =
  let open Arg in
  let array = info [ "a"; "array" ] ~doc:"Serialize the contents to an array of strings." in
  let list = info [ "l"; "list" ] ~doc:"Serialize the contents to a list of strings." in
...
let output =
  ...
  let pp ppf = function
    | None -> Fmt.string ppf "-"
    | Some filename -> Fmt.string ppf filename in
  let open Arg in
  value & opt (conv (parser, pp)) None & info [ "o"; "output" ] ~doc ~docv:"FILENAME"
...
let cmd =
  let doc = "Crunch some files into an OCaml file which can be statically \
             linked with an OCaml program. The OCaml program is able to obtain \
             the contents of these files then without I/O operations." in
  let info = Cmd.info "crunch" ~doc in
  Cmd.v info term
