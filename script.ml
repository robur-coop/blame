...
let bootf fmt =
  let fn txt =
    ...
    El.append_children boot [ pre ]; pre in
  Fmt.kstr fn fmt
...
let on_input documents bm25 _ev =
  ...
  let run () =
    ...
    let scores =
      ...
      List.iter fn lst; tbl in
    let topics = Brr.El.fold_find_by_selector ~root:topics_list (fun el acc -> el :: acc) (jstrf ".topic") [] in
    if Hashtbl.length scores <= 0
    then
      ...
      let fn acc el =
        ...
        | Some score -> El.set_class hidden false el; (score, el) :: acc
        | None -> El.set_class hidden true el; acc in
      let visible_divs = List.fold_left fn [] topics in
      let visible_divs = List.sort (fun (a, _) (b, _) -> Float.compare b a) visible_divs in
...
let progress total =
  ...
  let display () =
    ...
    let str = Fmt.str "> %d/%d document(s) downloaded (%d%%)" !current total percent in
    El.set_children el El.[txt' str] in
  reporter, display
...
let run () =
  ...
  let rec go = function
    ...
    | x :: r ->
        ...
        let div = El.div ~at:At.[ class' (jstrf "topic"); id (jstrf "%s" x.Format.uid) ]
  ...
  let fn uid =
    ...
    reporter 1; display (); Fut.return stem in
